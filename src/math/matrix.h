...
struct mat4 {
    ...
    inline vec4& operator[](uint32_t index) {
        return columns[index];
    }

    inline const vec4& operator[](uint32_t index) const {
        return columns[index];
    }
    ...
    inline static mat4 orthographic(float left, float right, float bottom, float top, float near, float far) {
        mat4 result;
    ...
    inline static mat4 look_at(vec3 position, vec3 target, vec3 up) {
    ...
    inline static mat4 translate(vec3 position) {
        mat4 result(1.0f);
    ...
    inline static mat4 rotate(float angle, vec3 axis) {
    ...
    inline static mat4 scale(vec3 value) {
        mat4 result(1.0f);
        result[0][0] = value.x;
        result[1][1] = value.y;
        result[2][2] = value.z;
